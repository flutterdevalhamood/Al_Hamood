...
class TyreReplacementPictureUploadScreen extends StatefulWidget {
  const TyreReplacementPictureUploadScreen({super.key});

  @override
  State<TyreReplacementPictureUploadScreen> createState() =>
      _TyreReplacementPictureUploadScreenState();
}

class _TyreReplacementPictureUploadScreenState
    extends State<TyreReplacementPictureUploadScreen> {
  ...
  Future<void> _uploadPictures() async {
    final tyreReplacementId =
        _tyreReplacementController.tyreOriginalData?[0]['id'];
    if (_selectedImages.isEmpty) {
      showErrorSnack('Please select at least one image to upload');
      return;
    }

    if (tyreReplacementId == null) {
    ...
    try {
      ...
      bool success = await _tyreReplacementController
          .postTyreReplacementPictureUpload(
            id: tyreReplacementId,
            files: imageFiles,
          );
